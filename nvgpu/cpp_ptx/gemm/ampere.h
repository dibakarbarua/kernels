...
#pragma unroll
    ...
    auto ldmatrix_load = [&](T *tile_sA, T *tile_sB, int k_step, int reg_idx)
    {
        ...
        int ldsm_a_offset = ldsm_a_row * 64 + ldsm_a_col_sw * 8;
        int ldsm_b_row = ldsm_row + k_step * 16;
        int ldsm_b_col = ldsm_col * 2 + warp_col;
#pragma unroll
        for (int i = 0; i < 4; ++i)
        {
            int col = ldsm_b_col + i * 4;
            int col_sw = (ldsm_b_row % 8) ^ col;
            int offset = ldsm_b_row * 128 + col_sw * 8;
            ldmatrix_x4_trans(tile_sB + offset, reg_b + reg_idx + i * 4);
        }
#pragma unroll
        for (int i = 0; i < 4; ++i)
        {
            ldmatrix_x4(tile_sA + ldsm_a_offset + i * 2048, reg_a + reg_idx + i * 4); // i + 1, row + 32
        }
    };
...
#pragma unroll
    for (int i = threadIdx.x; i < num_vectors; i += NumThreads)
    {
        ...
        float2 c_old = __ldg(reinterpret_cast<float2 *>(gC + row * N + col));
